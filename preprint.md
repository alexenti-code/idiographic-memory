...
Reference implementation in development under the former working name (`github.com/alexenti-code/matryoshka` — theory, stand, experiments; `github.com/alexenti-code/matryoshka-mmi` — prototype of memory acts and friction); both will be consolidated under the PlastFormer name. License: Apache 2.0. Russian-language research essays (nos. 21–24) at aura.kim document the architectural reasoning that led here; they are commentary, not the claim. The names PlastFormer and idiographic memory were checked for collisions prior to this draft; a final exact-match search will be repeated immediately before submission.
