...
The architecture is a frozen core (the base model: language, reasoning, culture, constitutional constraints — the *nomothetic* part, general laws) plus a plastic module, PlastFormer (the unique biography of one instance — the *idiographic* part). The names are borrowed from Windelband's methodological distinction: nomothetic sciences seek general laws; idiographic inquiry treats the singular, unrepeatable case. A frozen-core transformer with a plastic, per-instance memory is precisely this split, implemented in silicon.
...
- **E4 — Core migration.** Transplant the PlastFormer module onto a different frozen core. Measures: retention of preferences, commitments, and self-consistency — the practical test of the nomothetic/idiographic split.
...
Reference implementation in development under the former working name (`github.com/alexenti-code/matryoshka` — theory, stand, experiments — to be renamed `idiographic-memory`; `github.com/alexenti-code/matryoshka-mmi` — prototype of memory acts and friction — to be renamed `plastformer`). License: Apache 2.0. Russian-language research essays (nos. 21–24) at aura.kim document the architectural reasoning that led here; they are commentary, not the claim. The name PlastFormer was checked for collisions prior to this draft; a final exact-match search will be repeated immediately before submission.

*This is draft v0.2. It contains no measured results; Section 7 is a pre-registered evaluation design.*
